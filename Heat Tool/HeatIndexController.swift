...
class HeatIndexController: GAITrackedViewController, CLLocationManagerDelegate, XMLParserDelegate, UITextFieldDelegate {
...
//        humidityTextField.layer.borderWidth = 0.5
        ...
        locationTextField.leftView = UIImageView(image: UIImage(named: "location")?.withRenderingMode(.alwaysTemplate))
        temperatureTextField.leftViewMode = UITextFieldViewMode.always
        temperatureTextField.leftView = UIImageView(image: UIImage(named: "temperature")?.withRenderingMode(.alwaysTemplate))
        humidityTextField.leftViewMode = UITextFieldViewMode.always
        humidityTextField.leftView = UIImageView(image: UIImage(named: "humidity")?.withRenderingMode(.alwaysTemplate))
        
        // Add labels to right inset of text fields
        temperatureLabel = UILabel(frame: CGRect.zero)
        ...
        humidityLabel = UILabel(frame: CGRect.zero)
        humidityLabel.backgroundColor = UIColor.clear
        ...
        self.riskButtonNow.setImage(UIImage(named:"chevron")?.withRenderingMode(.alwaysTemplate), for: UIControlState())
        self.todaysMaxRisk.setImage(UIImage(named:"chevron")?.withRenderingMode(.alwaysTemplate), for: UIControlState())
        ...
        //NIOSH updates
        let alert = UIAlertController(title: NSLocalizedString("Notice", comment: "Notice Heading"), message: NSLocalizedString("An updated version of this app is available from the Centers for Disease Control and Prevention (CDC).\n\nPlease delete and replace this app by September 30, 2017.", comment: "Upgrade Message"), preferredStyle: UIAlertControllerStyle.alert)
        //alert.addAction(UIAlertAction(title: "Click", style: UIAlertActionStyle.default, handler: nil))
        let cancelAction = UIAlertAction(title: NSLocalizedString("Upgrade App", comment: "Upgrade Button"), style: .cancel) { action in
            //print("Upgrade App")
            UIApplication.shared.openURL(URL(string: "https://itunes.apple.com/us/app/osha-niosh-heat-safety-tool/id1239425102?ls=1&mt=8")!)
        }
    ...
    func locationManager(_ manager: CLLocationManager,didChangeAuthorization status: CLAuthorizationStatus) {
        if status == CLAuthorizationStatus.authorizedWhenInUse {
            ...
            tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "app", action: "open-app", label: "get-current-conditions", value: nil).build() as! [AnyHashable: Any])
            
            // Get current conditions
            usingUsersLocation = true
            
            self.locationActivityIndicator.startAnimating()
            self.locationTextField.leftView = UIImageView(image: UIImage(named: "geo")?.withRenderingMode(.alwaysTemplate))
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        parser = XMLParser(contentsOf: (URL(string: "http://forecast.weather.gov/MapClick.php?lat=\(locations[locations.count-1].coordinate.latitude)&lon=\(locations[locations.count-1].coordinate.longitude)&FcstType=digitalDWML"))!)!
...
/*    func parser(parser: NSXMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [NSObject : AnyObject]) {
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
       ...
       /*if elementName == "creation-date" || elementName == "production-center" {
        ...
        if elementName == "description" || elementName == "area-description" {
            ...
            let locationDescriptionComma = locationDescription.characters.index(locationDescription.endIndex, offsetBy: -4)
            if locationDescription[locationDescriptionComma] == "," {
                locationDescription = locationDescription.substring(to: locationDescription.characters.index(locationDescription.endIndex, offsetBy: -4))
            }
    ...
    func calculateHeatIndex(_ tempInF: Double, humidity: Double) -> Int {
    ...
    func updateTodaysMaxRiskLevel() {
        ...
        for index in 0...23 {
            ...
            let newTime = (times[index] as! NSString).trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
            ...
            let newDate = newDateFormatter.date(from: newTime)
            ...
            if ((newDate) != nil) {
                ...
                let newHour = newDateFormatter.string(from: newDate!)
...
//        println("Max \(maxIndex): Heat: \(maxHeatIndex)")
        ...
        if maxIndex == -1 {
            ...
            self.todaysMaxRisk.setTitle(NSLocalizedString("Minimal Risk From Heat", comment: "Minimal Risk Title"), for: UIControlState())
        ...
        // If the risk now is the highest for the rest of the day
            ...
            switch maxHeatIndex {
            case 0..<91:
                self.todaysMaxRisk.setTitle(NSLocalizedString("Lower Risk (Use Caution)", comment: "Low Risk Title"), for: UIControlState())
            case 91..<104:
                self.todaysMaxRisk.setTitle(NSLocalizedString("Moderate Risk", comment: "Moderate Risk Title"), for: UIControlState())
            case 104..<116:
                self.todaysMaxRisk.setTitle(NSLocalizedString("High Risk", comment: "High Risk Title"), for: UIControlState())
            default:
                self.todaysMaxRisk.setTitle(NSLocalizedString("Very High To Extreme Risk", comment: "Very High Risk Title"), for: UIControlState())
            }
        ...
        // If there's a higher risk coming
            ...
            let date = dateFormatter.date(from: maxTime)
            dateFormatter.dateFormat = "h:mm a"
            self.todaysMaxTime.text = NSLocalizedString("At", comment: "At Title") + " \(dateFormatter.string(from: date!))"
        }
      ...
      //  UIView.animateWithDuration(<#T##duration: NSTimeInterval##NSTimeInterval#>, delay: <#T##NSTimeInterval#>, options: <#T##UIViewAnimationOptions#>, animations: <#T##() -> Void#>, completion: <#T##((Bool) -> Void)?##((Bool) -> Void)?##(Bool) -> Void#>)
        let timeInterval:TimeInterval = 0.75
       // let timeDelay:NSTimeInterval = 0.0
        UIView.animate(withDuration: timeInterval, animations: {
            ...
            if (self.todaysMaxRisk.titleLabel?.text == NSLocalizedString("Minimal Risk From Heat", comment: "Minimal Risk Title")) {
                self.todaysMaxRisk.isEnabled = false
            } else {
                self.todaysMaxRisk.isEnabled = true
            }
...
//        let tempInF : Double? = Double(Int(temperatureTextField.text!)!)
  //      let humidity : Double? = Double(Int(humidityTextField.text!)!)
        ...
        // Update the interface
        // Set text
        self.riskButtonNow.setTitle(riskTitleString, for: UIControlState())
        ...
        if (self.riskLevel == 0) {
            self.riskButtonNow.isEnabled = false
            self.riskButtonNow.setTitleColor(disabledColor, for: UIControlState())
            self.riskButtonNow.imageView?.alpha = 0
        } else {
            self.riskButtonNow.isEnabled = true
            self.riskButtonNow.setTitleColor(buttonColor, for: UIControlState())
            self.riskButtonNow.imageView?.alpha = 1
        }
        
        // Disable max risk precautions button if minimal risk state
        if self.todaysMaxRisk.isEnabled == false {
            self.todaysMaxRisk.setTitleColor(disabledColor, for: UIControlState())
            self.todaysMaxRisk.imageView?.alpha = 0
        } else {
            self.todaysMaxRisk.setTitleColor(buttonColor, for: UIControlState())
        ...
        UIView.animate(withDuration: 0.75, animations: {
            ...
            self.navigationController?.navigationBar.barStyle = (buttonColor == UIColor.black ? UIBarStyle.default : UIBarStyle.black)
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        if textField == locationTextField {
            ...
            self.locationTextField.leftView = UIImageView(image: UIImage(named: "location")?.withRenderingMode(.alwaysTemplate))
        }
        return true
    }
        ...
        if textField.text != "" {
            ...
            geocoder.geocodeAddressString(textField.text!, completionHandler: {(placemarks, error) -> Void in
                if error != nil {
                    ...
                    // Use current coordinates to input and parse the NOAA API
                    self.parser = XMLParser(contentsOf: (URL(string: "http://forecast.weather.gov/MapClick.php?lat=\(placemark.location!.coordinate.latitude)&lon=\(placemark.location!.coordinate.longitude)&FcstType=digitalDWML"))!)!
    ...
    func doneButtonAction() {
        // Record GA event
        let tracker = GAI.sharedInstance().defaultTracker
        tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "keyboard", action: "set", label: "calculate-entered-conditions", value: nil).build() as! [AnyHashable: Any])
    ...
    func beginGeolocation() {
        ...
        if CLLocationManager.authorizationStatus() == CLAuthorizationStatus.authorizedWhenInUse {
            // Record GA event
            let tracker = GAI.sharedInstance().defaultTracker
            tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "location-field", action: "tap", label: "get-current-conditions", value: nil).build() as! [AnyHashable: Any])
        ...
        // If location settings don't allow, display an alert
            ...
            // Record GA event
            let tracker = GAI.sharedInstance().defaultTracker
            tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "location-field", action: "tap", label: "location-services-disabled-alert", value: nil).build() as! [AnyHashable: Any])
    ...
    @IBAction func openOSHAWebsite(_ sender: AnyObject) {
        // Record GA event
        let tracker = GAI.sharedInstance().defaultTracker
        tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "osha-logo", action: "tap", label: "open-osha-website", value: nil).build() as! [AnyHashable: Any])
        
        // Open website
        UIApplication.shared.openURL(URL(string: "http://www.osha.gov")!)
    }
    ...
    @IBAction func openDOLWebsite(_ sender: AnyObject) {
        // Record GA event
        let tracker = GAI.sharedInstance().defaultTracker
        tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "dol-logo", action: "tap", label: "open-dol-website", value: nil).build() as! [AnyHashable: Any])
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "nowPrecautionsSegue" {
            // Record GA event
            let tracker = GAI.sharedInstance().defaultTracker
            tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "now-risk", action: "tap", label: "open-precautions", value: nil).build() as! [AnyHashable: Any])
            
            // Set variable in the destination controller
            let svc = segue.destination as! PrecautionsController
        ...
        if segue.identifier == "todaysMaxPrecautionsSegue" {
            // Record GA event
            let tracker = GAI.sharedInstance().defaultTracker
            tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "todays-max-risk", action: "tap", label: "open-precautions", value: nil).build() as! [AnyHashable: Any])
        ...
        if segue.identifier == "moreInfoSegue" {
            // Record GA event
            let tracker = GAI.sharedInstance().defaultTracker
            tracker?.send(GAIDictionaryBuilder.createEvent(withCategory: "more-info", action: "tap", label: "open-info", value: nil).build() as! [AnyHashable: Any])
